...
struct ContentView: View {
    ...
    // Reference to Objective-C CalculatorLogic class
    ...
    func evaluateExpression(_ input: String) -> String {
        ...
        while let range = findInnermostParentheses(in: expression) {
            let inner = String(expression[range])
            let content = String(inner.dropFirst().dropLast())
            if content.contains("+") || content.contains("-") || content.contains("*") || content.contains("/") {
                ...
                if range.lowerBound > expression.startIndex {
                    ...
                    if precedingChar.isLetter {
                        // Likely outer parentheses of a trig function; do not remove it since letter's only appear in trig functions
                        break
                    }
                }
                // Else, remove redundant parentheses
                let evaluatedInner = evaluateArithmeticExpression(content)
    ...
    // Handles evaluation of artihmetic expressions using calculator logic class or reverts to NSExpression
    func evaluateArithmeticExpression(_ expression: String) -> String {
        let operators = ["+", "-", "*", "/"]
        var operatorFound: String? = nil
        var components: [String] = []
        
        // Check if the expression consists of exactly two numbers separated by a single operator
        for op in operators {
            let parts = expression.split(separator: Character(op)).map { String($0) }
            if parts.count == 2 {
                components = parts
                operatorFound = op
                break
        ...
        if let op = operatorFound, components.count == 2,
           let num1 = Double(components[0]), let num2 = Double(components[1]) {
            var result: Double = 0.0
            switch op {
            case "+":
                result = calcLogic.add(num1, with: num2)
            case "-":
                result = calcLogic.subtract(num1, with: num2)
            case "*":
                result = calcLogic.multiply(num1, with: num2)
            case "/":
                result = calcLogic.divide(num1, with: num2)
            default:
                return "Invalid Input"
            }
        ...
        // If not a simple two-number operation, use NSExpression
    ...
    // Inserts implicit multiplication operators ex. "cos(5)tan(5)" --> "cos(5)*tan(5)"
    ...
    // Process trig functions and replace calls like sin(x), cos(x), tan(x) with computed values
    func processTrigFunctions(_ expression: String) -> String {
        ...
        while true {
            ...
            // Call to calculator logic class to handle computation of trig function
        ...
        // If any unresolved trig calls remain, flag as invalid input
    ...
    // Removes redundant outer parentheses from a trig arguments ex. "((2))" becomes "2"
    ...
    var body: some View {
        if verticalSizeClass == .regular {
            VStack(spacing: 20) {
                ...
                // Calculator Buttons Grid for portrait phone view
                VStack(spacing: 10) {
                    ForEach([["7", "8", "9", "+"],
                             ["4", "5", "6", "-"],
                             ["1", "2", "3", "*"],
            ...
            VStack(spacing: 20) {
                ...
                // Calculator Buttons Grid for landscape view
                VStack(spacing: 10) {
                    ForEach([
